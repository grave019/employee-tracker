...
viewSomething = () => {
  ...
  // Menu section for viewing departments
viewDepartments = () => {
  ...
  // Roles section for viewing menu
viewRoles = () => {
    connection.query(
      ...
      .prompt([
        {
          name: "update",
          type: "list",
          message: "Choose an option to update:",
...
// Department section of deleting menu
deleteDepartment = () => {
  let departmentOptions = [];
    ...
    .then((answer) => {
      for (i = 0; i < departmentOptions.length; i++) {
        if (answer.deleteDepartment === departmentOptions[i].name) {
          ...
          console.log(
            "Department: " + answer.deleteDepartment + " Deleted Succesfully"
          );
        }
      }
      getDepartments();
...
// Employee section of deleting menu
deleteEmployee = () => {
  let employeeOptions = [];
  for (var i = 0; i < employees.length; i++) {
    employeeOptions.push(Object(employees[i]));
    ...
    .then((answer) => {
      for (i = 0; i < employeeOptions.length; i++) {
        if (answer.deleteEmployee === employeeOptions[i].Employee_Name) {
          ...
          connection.query(`DELETE FROM employee Where id = ${newChoice}`),
            (err, res) => {
              if (err) throw err;
            };
          console.log(
            "Employee: " + answer.deleteEmployee + " Deleted Succesfully"
          );
        }
      }
      getEmployees();
